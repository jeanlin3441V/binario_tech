...
// 1. GET /api/v1/veiculos - Listar todos os veiculos (Suporte filtro por status query param)
...
// 2. GET /aí/v1/veiculos/:id - Buscar veiculo por ID
...
app.patch('/api/v1/veiculos/:id/status', (req, res) => {
	...
	res.status(200).json({ mensagem: "status atualizados com sucesso!", veiculo});
});
...
// 6. PUT /api/v1/veiculos/:id - Substituir integralmente os dados de um veiculo
app.put('/api/v1/veiculos/:id', (req, res) => {
    ...
    if (!placa || !montadora || !modelo || !status) {
        return res.status(400).json({ erro: "Todos os campos ('placa', 'montadora', 'modelo', 'status') sao obrigatorios para atualizacao completa (PUT)." });
    }

    // Substituição integral do registro
    veiculos[index] = {
        id,
        placa,
        montadora,
    ...
    res.status(200).json({ mensagem: "Veiculo atualizado com sucesso!", veiculo: veiculos[index] });
});

app.listen(PORT, () => {
	console.log(`[Binario Tech] API de Fronte rodando em http://localhost:${PORT}`);
});
